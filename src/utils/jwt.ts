...
export function signAccessToken(sub: number, role: JwtRole, name?: string) {
  return jwt.sign({ role, name }, env.jwtSecret, { subject: String(sub), expiresIn: env.jwtExpiresIn });
}

export function signRefreshToken(sub: number, role: JwtRole) {
  return jwt.sign({ role, typ: 'refresh' }, env.refreshJwtSecret, { subject: String(sub), expiresIn: env.refreshExpiresIn });
}
